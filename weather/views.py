...
def cron(request):
    ...
    if user is not None:
        ...
        if resp is not None:
            ...
            owm.city = City.objects.get(name=pj['city']['name'])
            ...
            return render(request, 'weather/cronsucc.html', context)
        else:
            return render(request, 'weather/cronfail.html', {'error': 'req fail'})
    else:
        return render(request, 'weather/cronfail.html', {'error': 'auth fail', 'u': username, 'p': password})
